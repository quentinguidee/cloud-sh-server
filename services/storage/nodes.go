...
func GetBucketNode(tx *sql.Tx, uuid string) (Node, IServiceError) {
	request := "SELECT uuid, name, type, bucket_id FROM buckets_nodes WHERE uuid = ?"

	var node Node
	err := tx.QueryRow(request, uuid).Scan(&node.Uuid, &node.Name, &node.Type, &node.BucketId)
	if err != nil {
...
func GetBucketNodes(tx *sql.Tx, parentUuid string) ([]Node, IServiceError) {
	request := `
		SELECT uuid, name, type, bucket_id
	...
	for rows.Next() {
		var node Node
		err := rows.Scan(&node.Uuid, &node.Name, &node.Type, &node.BucketId)
		if err != nil {
...
func GetBucketNodeParent(tx *sql.Tx, nodeUuid string) (Node, IServiceError) {
	request := `
		SELECT nodes.uuid, name, type, bucket_id
		FROM buckets_nodes nodes, buckets_nodes_associations associations
	...
	var parent Node
	err := tx.QueryRow(request, nodeUuid).Scan(&parent.Uuid, &parent.Name, &parent.Type, &parent.BucketId)
	if err != nil {
		return Node{}, NewServiceError(http.StatusInternalServerError, err)
	}
...
func CreateBucketNode(tx *sql.Tx, name string, kind string, bucketId int) (Node, IServiceError) {
	node := Node{
		Uuid:     uuid.NewString(),
		Name:     name,
		Type:     kind,
